    ...
    <div className="grid gap-3 md:gap-4">
      ...
      >
        ...
        <div className="grid md:grid-cols-3 gap-3">
          ...
          <input className="border border-gray-300 rounded px-2 py-1 text-sm md:text-base" type="number" step="0.01" value={limit} onChange={e=>setLimit(Number(e.target.value))} placeholder="Limite mensal (R$)" />
          <button className="bg-green-600 text-white rounded px-3 py-2 hover:bg-green-700 text-sm md:text-base transition-colors">Salvar</button>
        </div>
            ...
            {editingId === b.id ? (
              <div className="grid gap-3">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={async () => {
                      if (editCategory && editLimit > 0) {
                        await updateBudget(b.id, { category: editCategory, limit: editLimit })
                        setEditingId(null)
